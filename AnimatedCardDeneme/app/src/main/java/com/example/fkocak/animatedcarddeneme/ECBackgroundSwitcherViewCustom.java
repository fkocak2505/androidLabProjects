 ...
 */
public class ECBackgroundSwitcherViewCustom extends ImageSwitcher {
    ...
    private BitmapWorkerTaskCustom mCurrentAnimationTask;

    public ECBackgroundSwitcherViewCustom(Context context, AttributeSet attrs) {
        super(context, attrs);
        inflateAndInit(context);
    }

    public ECBackgroundSwitcherViewCustom(Context context) {
        super(context);
        inflateAndInit(context);
    }
...
//    }
    ...
    public void cacheBackgroundAtPosition(ECPagerCustom pager, int position) {
        if (position >= 0 && position < pager.getAdapter().getCount()) {
            ...
            BitmapWorkerTaskCustom addBitmapToCacheTask = new BitmapWorkerTaskCustom(getResources(), mainBgImageDrawableResource);
            addBitmapToCacheTask.execute(position);
        }
    }

    public void updateCurrentBackground(ECPagerCustom pager, final AnimationDirection direction) {
    ...
    public void updateCurrentBackgroundAsync(ECPagerCustom pager, final AnimationDirection direction) {
        ...
        Integer mainBgImageDrawableResource = pager.getDataFromAdapterDataset(position).getMainBackgroundResource();
        if (mainBgImageDrawableResource == null) return;
        mCurrentAnimationTask = new BitmapWorkerTaskCustom(getResources(), mainBgImageDrawableResource) {
            @Override
